...
**Decision (user-confirmed scope): "Hard bounds, minor overlap"** — apply the *existing* minor-overlap
budget to hard bounds, for **all** minor-overlap-eligible roles. No new constant, no new helper,
no rect-geometry changes.
...
Change to allow a bounded overlap, reusing the existing `shouldAllowMinorOverlap` exactly as the
label-overlap path already does (`:317-322`):
...
Because the overlap budget is `MINOR_OVERLAP_HEIGHT_RATIO = 0.45 × labelHeight` — i.e. whitespace
level — a label hugs its anchor at **step 0** (no leader) when the hard-bound overlap is minor, and
is still displaced when the overlap is real. This is why the today-low (58.8°) places below at step 0
instead of `below+1`. The "631°" near-total overlap exceeds the budget, so forecast `LOW` there still
flips above (guarded by the `260612` test).

Mirror the same allowance in the curve-avoidance pre-pass gate `checkExactFitBlockers`
(`:621`, the `baseOverlapsHard` term: gate it through `shouldAllowMinorOverlap` against the base
intrusion overlap) so the pre-pass doesn't pre-emptively reject the below direction. The
post-displacement re-check in `tryExactFitForDirection` (`:705-706`) stays a hard reject (it only
runs after a deliberate displacement; keeping it strict avoids re-introducing leaders there).

### 2. (no shared-utils change)

`GraphLabelPlacementUtils.shouldAllowMinorOverlap` / `MINOR_OVERLAP_HEIGHT_RATIO` are reused as-is.
...
- **New shared test** `shared/src/test/kotlin/com/weatherwidget/shared/graph/TemperatureLabelHardBoundMinorOverlapTest.kt`
  ...
  1. `ACTUAL_LOW` at a valley + a `reservedHardBounds` rect overlapping its just-below slot **within
     budget** (≤ 0.45 × labelHeight) + curve filling above ⇒ assert `placedAbove == false`,
     `displacementSteps == 0`, `drawLeaderLine == false` (reproduces the fix; control without the
     allowance ⇒ `below+1`).
  2. Hard overlap **beyond** `MINOR_OVERLAP_HEIGHT_RATIO` ⇒ still displaced (no unbounded overlap).
  3. **Regression guard (`260612`)**: forecast `LOW` whose below-slot is *near-totally* covered by the
     hard bound + fillable above ⇒ `placedAbove == true` (must exceed the 0.45 budget so it still
     flips). Also re-run `TemperatureLabelFetchDotHardBoundsTest` unchanged — verify its synthetic
     overlap is large (> budget); if it was a thin overlap, the relaxed gate now (correctly) keeps the
     LOW below, so update that expectation deliberately and note it.
...
1. `./gradlew :shared:test --tests "*HardBoundMinorOverlap*" --tests "*FetchDotHardBounds*"`
