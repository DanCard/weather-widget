...
## Open decision (flag before coding)
The `H D` chips, zoom chip, and ☁ toggle on the secondary row are **hourly-view** controls too,
not just daily. Two ways to remove them from the *daily* header without breaking hourly:
- **(a) Conditional rendering** — keep the secondary row only when `showWeatherSummary` (hourly
  family), and never render it in DAILY. Daily gets the clean Android band; hourly keeps its
  zoom/cloud controls. Lowest risk, recommended.
- **(b) Remove globally** — delete the secondary row for all views and re-home zoom/cloud into the
  graph surface or Settings. Cleaner header everywhere, but larger change touching hourly UX.

Recommend **(a)** unless you want the hourly header reworked in the same pass.

## Out of scope
- Hourly-view *visual* redesign (only the daily header must match Android here; hourly keeps its
  current controls per decision (a)).
- Android-side changes — Android already looks correct.
