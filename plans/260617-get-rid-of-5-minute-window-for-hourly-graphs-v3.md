...
**Gap today:** the only convergence test
(`ActualTemperatureSeriesBuilderTest."single-day build reproduces daily aggregate high and low
exactly"`) compares `ActualsAggregator.aggregate` against `build().points` raw max/min — it stops
*before* `TemperatureExtrema.compute`, which is what actually selects/labels the hourly graph's
per-day high/low. So nothing asserts the hourly view's *displayed* numbers equal the daily bar's.
That gap is how the divergence shipped.

**Add a cross-pipeline integration test** (shared, pure JUnit — no Android) that runs one observation
set through BOTH real paths and asserts equality per day:
- Daily bar: `ActualsAggregator.aggregate(obs)` → `highTemp`/`lowTemp`.
- Hourly view: `ActualTemperatureSeriesBuilder.build(obs)` → `HourDataAssembler.assembleHourData(...)`
  → `TemperatureExtrema.compute(...)` → the values at `actualDailyHighIndices` / `actualDailyLowIndices`
  (read from `actualLabelTemps`).
- Assert the hourly labeled per-day high/low == the daily `highTemp`/`lowTemp` for each fully-visible
  past day. Cover a **multi-day window** and a **sharp off-5-min peak** that the old thinning would
  have dropped (so the test fails on the pre-change code and passes after).

This becomes the durable "daily == hourly" regression guard. Keep the existing builder-level test too.
