...
### 1. Reorder special labels by priority (LOW first) and remove pastHigh
Keep start/end labels for edge context, but change from `Set` to ordered `List` so priority controls draw order. Labels drawn first win — later labels that collide are skipped.

Remove `pastHighIndex` (redundant — if it coincides with daily high it's a duplicate; if not, it adds clutter).
...
| What | Action |
|------|--------|
| Daily low label | Keep (priority 1 — always drawn) |
| Daily high label | Keep (priority 2 — drawn if no overlap with low) |
| Start point label | Keep (priority 3 — drawn if no overlap) |
| End point label | Keep (priority 4 — drawn if no overlap) |
| Past high label | Remove (redundant) |
