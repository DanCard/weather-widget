...
**How it got there — and why it's in TWO places:** commit `72e5a033` ("Fix hourly graph showing
hindsight revisions for past hours") tried to avoid NWS's REPLACE-overwritten "hindsight revision" by
showing the "original prediction," but implemented that as the *earliest* snapshot — the 6–7-day-out,
least-accurate long-range forecast, not a day-ahead prediction. That same "earliest-for-past" mental
model was then **copied into a second selector**: `CurrentTemperatureResolver.pickBestForecast`
(`shared/.../widget/CurrentTemperatureResolver.kt:87`, introduced separately in commit `6325cab2`) also
does `if (dateTimeMs < nowMs) minByOrNull { fetchedAt } else maxByOrNull { fetchedAt }`. This is why the
live current-temp `estimate` reads ~75.7° / `estAtObs` ~76.7° while the actual fresh forecast is 74° —
the interpolation anchors on the same stale 78–79° earliest snapshots. Both sites must be corrected.
...
### 1b. `shared/src/main/kotlin/com/weatherwidget/widget/CurrentTemperatureResolver.kt` (second site)
`pickBestForecast` (L87–88): collapse the past/future branch to **always** pick the freshest snapshot —
`return candidates.maxByOrNull { it.fetchedAt }`. Drop the `if (dateTimeMs < nowMs) minByOrNull{...}`
past-hour case. This stops the current-temp interpolation from anchoring on stale 6-day-old snapshots, so
the live `estimate` tracks the latest forecast (≈74° here) instead of ≈75.7°. The resolver's unit tests
use a single snapshot per hour (`CurrentTemperatureResolverTest.kt`), so `min→max` is behavior-neutral
there; still re-run them.
...
1. **Unit tests:**
   ...
   - `./gradlew :app:testDebugUnitTest --tests "*CurrentTemperatureResolverTest*"` (resolver fix is
     behavior-neutral on existing single-snapshot tests; confirm green)
2. **Desktop (real app):** rebuild + restart via `scripts/buildStart.sh`, then `touch
   ~/.local/share/weather-widget/.show`, capture the panel
   (`DISPLAY=:0 import -window root /tmp/.../panel.png && convert ... panel.jpg`) and confirm (a) the hourly
   graph's past-hours peak now reads **74°**, not 78°, and (b) the latest `CURR_TEMP_RESULT` log
   `estimate` drops from ~75.7° toward ~74°.
