...
find logs/ -mtime +14 -delete 2>/dev/null

# Run gradle: full output to log file, screen only on failure
./gradlew :app:testDebugUnitTest $FORCE_FLAG --console=plain >"$UNIT_LOG" 2>&1
...
if [ "$EXIT_CODE" -ne 0 ] && [ "$TOTAL" -gt 0 ]; then
    echo ""
    echo -e "${RED}Gradle output (last 40 lines):${NC}"
    tail -40 "$UNIT_LOG"
fi
