...
/**
 * Places cloud layers as curves whose line is made of repeated tiny glyphs — `l`, `m`, and `h` —
 * spaced like a dash pattern.
 *
...
object CloudLayerGlyphPlacer {
    ...
    /**
     * Distinct phases for observed trails so low/mid/high interleave with each other and with the
     * forecast middle/high trails instead of overprinting.
     */
    const val MID_ACTUAL_PHASE = 0.25f
    const val HIGH_ACTUAL_PHASE = 0.75f
    const val LOW_ACTUAL_PHASE = 0.125f
     ...
     */
    // Both trails use the same small type size. Forecast remains visually distinct through its
    // neutral color and bold weight; actual remains pink and normal weight.
    const val GLYPH_SIZE_DP = 4.5f
    const val ACTUAL_GLYPH_SIZE_DP = GLYPH_SIZE_DP

    // 2026-08-27 visual review: add 30% breathing room to the former 13dp forecast and 20dp actual
    // steps. Keep separate constants because actuals remain intentionally sparser.
    const val GLYPH_STEP_DP = 16.9f
    const val ACTUAL_GLYPH_STEP_DP = 26f
     ...
     * @param suppressMatchingTotal when true, omit a glyph whose band exactly equals total. This
     *   is an actual-trail presentation rule: the solid actual line already shows the controlling
     *   layer, so glyphs only explain the other vertical layers.
     */
    fun place(
        ...
        suppressMatchingTotal: Boolean = false,
    ): List<LayerGlyph> {
        ...
        for (i in 0 until vertices.size - 1) {
            ...
            while (untilNext <= segment - travelled) {
                ...
                val totalCover = interpolate(a.totalCover, b.totalCover, t)
                val matchingTotal = suppressMatchingTotal && totalCover != null && cover == totalCover
                if (cover >= minCover && !matchingTotal) {
    ...
    /**
     * The glyph's ink box as multiples of [GLYPH_SIZE_DP]: wide enough for `m` (the widest layer
     * letter) and tall enough for the font box, both biased slightly generous.
     *
     ...
     * font stacks asked for the width of a tiny bold `m` do not have to agree — [glyphBounds]
     ...
     *
     * Being a pixel out on a tiny glyph costs nothing, and erring large errs toward keeping text
     * off the trail.
     */
