...
class ActualsLoneStationGuardTest {
    ...
    fun `lone sparse station cannot set the daily low when other stations report that day`() {
        ...
        assertTrue("low ${daily.lowTemp} should come from the officials, not the lone 48.99 outlier", daily.lowTemp >= 54f)
        assertTrue("low ${daily.lowTemp} should stay near the officials' 54.0 floor", daily.lowTemp <= 55f)
        assertTrue("high ${daily.highTemp} unaffected by the guard", daily.highTemp in 74f..76f)
    ...
    fun `hourly build and daily aggregate agree on the low when a lone station is suppressed`() {
        ...
        assertEquals("hourly graph min must equal the stored daily low", daily.lowTemp, actualTemps.min(), 0.001f)
        assertEquals("hourly graph max must equal the stored daily high", daily.highTemp, actualTemps.max(), 0.001f)
    }
    ...
    fun `single station source still yields daily extremes`() {
        ...
        assertEquals(56f, daily.lowTemp, 0.001f)
        assertEquals(70f, daily.highTemp, 0.001f)
    }
