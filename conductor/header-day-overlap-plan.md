...
1. **Create formatting helper:** Create a new utility function (e.g., `HeaderFormatter.formatSourceIndicator(centerTime: LocalDateTime, now: LocalDateTime, sourceName: String, widthDp: Int): String`) that calculates the header text based on the available `widthDp`.
   - `widthDp >= 380`: Full day name + source (e.g., "Sunday • NWS").
   - `300 <= widthDp < 380`: Short day name + source (e.g., "Sun • NWS").
   - `widthDp < 300`: Source only (e.g., "NWS").
...
- Observe the day name disappears at `widthDp < 300dp`.
- Observe the abbreviated day name between `300dp` and `379dp`.
- Observe the full day name at `widthDp >= 380dp`.
