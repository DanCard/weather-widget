...
interface ForecastDao {
    @Query("SELECT * FROM forecasts ORDER BY fetchedAt DESC LIMIT 1")
    ...
    @Query(
        ...
        AND fetchedAt = (
            SELECT MAX(fetchedAt) FROM forecasts f2
    ...
    @Query(
        ...
        AND targetDate >= :startDate
        AND targetDate <= :endDate
        ORDER BY targetDate ASC, forecastDate DESC, fetchedAt DESC
    """,
    )
