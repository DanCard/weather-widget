...
class TemperatureFetchDotColorTest {
    ...
    @Test
    fun `fetch dot labels use actual line color`() {
        ...
        assertEquals("Value label color should match actual line color (with alpha)", expectedValueColor, debugResult?.valueColor)
        assertEquals("Staleness label color should match actual line color (no alpha)", expectedStalenessColor, debugResult?.stalenessColor)
    }
