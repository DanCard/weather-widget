...
class DailyViewGraphClickAlignmentTest {
    ...
    @Test
    fun `setupGraphDayClickHandlers compacts touch zones when first day is missing`() = runBlocking {
        ...
        // Check visibility: only the two rendered day zones should be VISIBLE
        for (i in 0 until 2) {
        ...
        // Verify click on the first rendered zone opens Today
        ...
        assertEquals(1, lastBroadcast.getIntExtra("index", -1))
        
        // Verify click on the second rendered zone opens Tomorrow
        val zone1 = applied.findViewById<View>(R.id.graph_day2_zone)
        zone1.performClick()
        val tomorrowBroadcast = broadcasts.last()
        assertEquals("com.weatherwidget.ACTION_DAY_CLICK", tomorrowBroadcast.action)
        assertEquals(tomorrowStr, tomorrowBroadcast.getStringExtra("date"))
        assertEquals(2, tomorrowBroadcast.getIntExtra("index", -1))

        // Verify a hidden trailing zone does NOTHING
        val hiddenZone = applied.findViewById<View>(R.id.graph_day3_zone)
        val broadcastCountBefore = broadcasts.size
        hiddenZone.performClick()
