...
class ForecastSnapshotDeduplicationTest {
    ...
    fun setup() {
        ...
        val forecastRepo = ForecastRepository(context, db.forecastDao(), db.hourlyForecastDao(), db.hourlyActualDao(), db.appLogDao(), mockk(), mockk(), mockk(), mockk(relaxed = true), mockk(relaxed = true), db.climateNormalDao(), db.observationDao())
        val currentRepo = CurrentTempRepository(context, db.currentTempDao(), db.observationDao(), db.hourlyActualDao(), db.hourlyForecastDao(), db.appLogDao(), mockk(), mockk(), mockk(), mockk(relaxed = true), mockk(relaxed = true), TemperatureInterpolator())
