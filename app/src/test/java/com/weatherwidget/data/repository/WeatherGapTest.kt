...
class WeatherGapTest {
    ...
    fun setup() {
        ...
        val forecastRepo = ForecastRepository(context, forecastDao, hourlyForecastDao, mockk(relaxed = true), appLogDao, nwsApi, openMeteoApi, weatherApi, mockk(relaxed = true), widgetStateManager, climateNormalDao, observationDao)
        val currentRepo = CurrentTempRepository(context, mockk(relaxed = true), observationDao, mockk(relaxed = true), hourlyForecastDao, appLogDao, nwsApi, openMeteoApi, weatherApi, mockk(relaxed = true), widgetStateManager, temperatureInterpolator)
