...
class WeatherHistoryConditionTest {
    ...
    @Test
    fun `fetchDayObservations calculates Sunny for Fair observations`() =
        runTest {
            val date = LocalDate.now()
    ...
    @Test
    fun `fetchFromNws uses forecast summary for todays condition`() =
        runTest {
            val lat = 37.422
            val lon = -122.0841
            ...
            coEvery { nwsApi.getForecast(any()) } returns
                listOf(
    ...
    @Test
    fun `fetchFromNws uses tonight forecast condition when todays first period is nighttime`() =
        runTest {
            val lat = 37.422
            val lon = -122.0841
            val today = LocalDate.now()
    ...
    @Test
    fun `fetchFromNws assigns malformed startTime periods to parsed ISO date prefix`() =
        runTest {
            val lat = 37.422
            val lon = -122.0841
