...
class ForecastRoundingTest {
    ...
    fun setup() {
        ...
        repository = ForecastRepository(context, db.forecastDao(), db.hourlyForecastDao(), db.hourlyActualDao(), db.appLogDao(), mockk(), mockk(), mockk(), mockk(relaxed = true), mockk(relaxed = true), db.climateNormalDao(), db.observationDao()
        )
    }
