...
class WeatherRepositoryRateLimitIntegrationTest {
    ...
    private fun buildRepo(
        ...
        val forecastRepo = ForecastRepository(context, db.forecastDao(), db.hourlyForecastDao(), db.hourlyActualDao(), db.appLogDao(), nwsApi, openMeteoApi, weatherApi, mockk(relaxed = true), widgetStateManager, db.climateNormalDao(), db.observationDao())
        val currentRepo = CurrentTempRepository(context, db.currentTempDao(), db.observationDao(), db.hourlyActualDao(), db.hourlyForecastDao(), db.appLogDao(), nwsApi, openMeteoApi, weatherApi, mockk(relaxed = true), widgetStateManager, TemperatureInterpolator())
